...
xlabel('Maximum queue capacity $Q$',fontsize=14)
ylabel('Expected utility',fontsize=16)
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
legend(loc=(0.6,0.5), ncol=1,fancybox=True,shadow=True)
...
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
legend(loc=(0.12,0.45), ncol=2,fancybox=True,shadow=True,fontsize=10)
ylim([0.05,0.43])
# legend(loc='best',fancybox=True)
locs, labels = plt.yticks()
plt.setp(labels, rotation=90)
